<template>
  <div class="container mx-auto p-4">
    <div class="w-full max-w-2xl mx-auto">
       <h1 class="text-2xl font-bold mb-6">{{ $t('dishesCategories.viewTitle') }}</h1>
       
        <div v-if="store.isLoading" class="text-center"><p>{{ $t('message.loading') }}</p></div>
        ...
        <div v-else-if="store.dishCategory" class="bg-white p-8 rounded-lg shadow-md">
            <div class="space-y-4">
              ...
              <div class="flex items-center"><span class="w-1/3 text-gray-500 font-semibold">{{ $t('field.color') }}</span><div class="w-8 h-8 rounded-full border" :style="{ backgroundColor: store.dishCategory.color }"></div></div>
              <div class="flex items-start"><span class="w-1/3 text-gray-500 font-semibold pt-2">{{ $t('field.logo') }}</span><img v-if="store.dishCategory.logo" :src="store.dishCategory.logo" class="w-24 h-24 rounded-lg object-cover border"></div>
            </div>
            
             <div class="mt-8 flex justify-end space-x-4">
                <NuxtLink :to="localePath('/dishes-categories')" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400">{{ $t('button.backToList') }}</NuxtLink>
                <NuxtLink :to="localePath(`/dishes-categories/${route.params.id}/edit`)" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">{{ $t('button.edit') }}</NuxtLink>
            </div>
        </div>
