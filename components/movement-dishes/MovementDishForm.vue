  ...
  <div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-x-6 gap-y-4 mb-8">
      ...
      <div class="md:col-span-2">
        <label class="block text-sm font-medium text-gray-700">{{ $t('movementDish.warehouse_from') }}</label>
        <select v-model="formData.warehouse_from" :disabled="isViewMode" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm disabled:bg-gray-100">
      ...
      <!-- warehouse_to.item -->
      <div class="md:col-span-2">
        <label class="block text-sm font-medium text-gray-700">{{ $t('movementDish.warehouse_to') }}</label>
        <select v-model="formData.warehouse_to" :disabled="isViewMode" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm disabled:bg-gray-100">
          <option :value="null" disabled>{{ $t('messages.select') }}</option>
          <option v-for="w in warehouseStore.warehouses" :key="w.id" :value="w.id">{{ w.name }}</option>
        </select>
      </div>
...
import type { MovementDishDetailRich } from '~/types/documents';
...
const props = defineProps<{
  initialData?: MovementDishDetailRich | null;
  isViewMode?: boolean;
...
const formData = ref<Partial<MovementDishDetailRich>>({
...
watch(() => props.initialData, (data) => {
  if (data) {
    ...
    if (typeof tempData.warehouse_from === 'object' && tempData.warehouse_from !== null) {
      tempData.warehouse_from = tempData.warehouse_from.id;
    }

    if (typeof tempData.warehouse_to === 'object' && tempData.warehouse_to !== null) {
      tempData.warehouse_to = tempData.warehouse_to.id;
    }
...
const formTitle = computed(() => {
  if (!props.initialData) {
    return t('movementDish.creating');
  }

  const formattedDate = formatDateForTitle(formData.value.date || '');

  if (locale.value === 'kz') {
    return `${formattedDate} жылғы қоймаға ауыстыру`;
  }
  return `Перемещение на склад от ${formattedDate}`;
});
...
function submit() {
  ...
  payload.movement_dish_items = payload.movement_dish_items.map(item => ({
    ...item,
